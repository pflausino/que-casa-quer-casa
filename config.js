/* =====================================================================
   CONFIGURAÇÃO — edite somente este arquivo para ligar o formulário
   à sua planilha do Google.

   Passo a passo completo está no README.md.
   Resumo:
   1. Crie uma planilha no Google Sheets.
   2. Em Extensões > Apps Script, cole o conteúdo de apps-script/Code.gs.
   3. Implante como "App da Web" (acesso: Qualquer pessoa).
   4. Copie a URL gerada (termina em /exec) e cole abaixo.
   ===================================================================== */

window.WEDDING_CONFIG = {
  // Cole aqui a URL do seu Web App do Apps Script (termina em /exec).
  // Enquanto estiver vazio, o formulário roda em MODO DEMONSTRAÇÃO
  // (mostra os dados no console e não envia para lugar nenhum).
  GOOGLE_SCRIPT_URL: "",
};
